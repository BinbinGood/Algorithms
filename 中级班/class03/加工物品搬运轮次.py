...
def MinWays(str):
    ...
    if sumstr % len(str) != 0:
        return -1
    avg = int(sumstr / len(str))  # 均值
    res = 0  # 存放最大的轮次
    sum = 0  # 累加和
    for i in range(len(str)):
        leftrest = avg * i - sum  # 左侧剩余
        rightrest = (len(str) - i - 1) * avg - (sumstr - sum - str[i])  # 右侧的剩余
        if leftrest < 0 and rightrest < 0:
            res = max(res, abs(rightrest) + abs(leftrest))
        else:
            res = max(res, max(abs(rightrest), abs(leftrest)))
        sum += str[i]
    return res
